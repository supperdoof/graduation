...
/**
 * Created by hadoop on 18-10-2.
 */
public class MyKmeansNew {
    ...
    private ArrayList<Integer> mapList;//存储聚类中心和数据点的映射关系
    private ArrayList<Integer> countList;//存储数据点的异常次数
     ...
     * 初始化异常次数集合
     *
     * @return 一个长度为dataSet.length的空数据的映射集合
     */
    private ArrayList<Integer> initCount() {
     ...
     * @return 一个长度为dataSet.length的空数据的映射集合
     */
    private ArrayList<Integer> initMap() {
        ArrayList<Integer> map = new ArrayList<Integer>();
        for (int i = 0; i < dataSetLength; i++) {
            map.add(0);
        }
...
//                sparseCenter[1] = sparseCenter[1]/(center.size() - 1);
    ...
    {
        ArrayList<Float> arrayList = new ArrayList<>(cluster.size());
        ...
        for(int i=0; i<cluster.size();i++)
        {
            ...
            for(int j=0; j<cluster.get(i).size();j++)
            {
                distinct += distance(cluster.get(i).get(j),center.get(i));
            }
            distinct /= cluster.get(i).size();
            arrayList.add(distinct);
        }

        for(int i=0; i<mapList.size();i++)
        {
            float distinct = distance(dataSet.get(i),center.get(mapList.get(i)));
            if(distinct >= arrayList.get(mapList.get(i)))
            {
    ...
    public void execute() {
        ...
        System.out.println("MyKmeansNew begins");
        kmeans();
        long endTime = System.currentTimeMillis();
        System.out.println("MyKmeansNew running time=" + (endTime - startTime)
                + "ms");
        System.out.println("MyKmeansNew ends");
